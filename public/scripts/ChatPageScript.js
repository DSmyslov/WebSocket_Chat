    ...
    if (websocket.readyState == 1) {
        ...
        if (text == '') {
            ...
            const data_to_send = {
                ...
                to: user_id ? 0 : 1, 
                user_sent_time: Date.now()
            }
            queue.push(data_to_send);
            if (queue.length == 1 && websocket.bufferedAmount == 0) {
...
const ws_onmessage = event => {
    ...
    if (parsedMessage.type && parsedMessage.type == 'changeMessageStatus') {
        let msg_to_cnahge = document.getElementById(parsedMessage.server_timestamp);
        msg_to_cnahge.getElementsByClassName('status')[0].innerHTML = parsedMessage.status
        return;
    }
    showMessage(parsedMessage);
    if (parsedMessage.to !== user_id) {
        queue.shift();
        if (websocket && websocket.readyState == 1 && queue.length) {
            websocket.send(JSON.stringify(queue[0]));
        }
    }
    else {
        // отправляем сообщение, чтобы изменить его статус у отправителя на "Доставлено"
        const data_to_send = {
            type: 'changeMessageStatus',
            to: user_id ? 0 : 1,
            message_id: parsedMessage.server_timestamp,
            status: 'Доставлено'
        }
        queue.push(data_to_send);
        if (queue.length == 1 && websocket.bufferedAmount == 0) {
