...
send_message.onclick = event => {
    ...
    if (websocket.readyState === 1) {
        ...
        if (text == '') alert('Нельзя отправить пустое сообщение')
        else {
            websocket.send(JSON.stringify({ message: text, from_nickname: show_nickname.innerHTML, to: user_id ? 0 : 1, user_sent_time: Date.now()}));
            message_text.value = ''
        }
    }
    else alert('Соединение с websocket сервером еще не установлено')
...
const showMessage = parsedMessage => {
    ...
    if (parsedMessage.to !== user_id) {
        ...
        new_msg_info.innerHTML = `<div class="status">Отправлено</div>
